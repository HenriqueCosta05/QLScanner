  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && pathname === `${API_PREFIX}/scans`) {
      try {
        ...
        const trimmedRepoRoot =
          typeof body.repoRoot === "string" ? body.repoRoot.trim() : "";
        if (!trimmedRepoRoot) {
          sendJson(res, 400, { error: "Field `repoRoot` is required." });
          return;
        }
        const scan = scanService.createScan({ repoRoot: trimmedRepoRoot });
