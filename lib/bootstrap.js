...
export async function ensureCodeQL(baseDir) {
  ...
  if (arch !== 'x64') {
    throw new Error(`Arquitetura não suportada: ${arch}. Apenas x64 é compatível com CodeQL CLI oficial.`);
  }

  // Verifica se já está no PATH
  try {
    const codeqlFromPath = which.sync('codeql', { nothrow: true });
    if (codeqlFromPath && isExecutable(codeqlFromPath)) {
